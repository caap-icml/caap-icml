...
bind_focus_out = "<FocusOut>"
bind_focus_in = "<FocusIn>"
bind_escape = "<Escape>"
...
class Annotator(BaseWindow):
    ...
    def fill_frame__xxyy(self, frame):
        ...
        button = tk.Button(subframe, height=1, text="<", command=lambda: self.bbox_coordinate_controller(self.x1, 'down'))
        button.pack(side='left')
        ...
        self.textbox_event_binder(textbox)
        textbox.pack(side='left')
        button = tk.Button(subframe, height=1, text="<", command=lambda: self.bbox_coordinate_controller(self.x2, 'down'))
        button.pack(side='left')
        ...
        self.textbox_event_binder(textbox)
        textbox.pack(side='left')
        button = tk.Button(subframe, height=1, text="▲", command=lambda: self.bbox_coordinate_controller(self.y1, 'down'))
        button.pack(side='left')
        ...
        self.textbox_event_binder(textbox)
        textbox.pack(side='left')
        button = tk.Button(subframe, height=1, text="▲", command=lambda: self.bbox_coordinate_controller(self.y2, 'down'))
        button.pack(side='left')
    ...
    def textbox_event_binder(self, textbox):
        textbox.bind(bind_focus_in, self.text_on_entry_focus)
        textbox.bind("<Return>", self.bbox_coodinate_textbox_on_enter_pressed)
        textbox.bind(bind_escape, self.text_on_escape)
        textbox.bind(bind_focus_out, self.text_on_escape)
    ...
    def fill_frame__type(self, frame):
        ...
        TYPE_DICT = {key: value for key, value in TYPE_DICT.items() if key < 100}  # Exclude key=100
        TYPE_DICT_1 = {k: TYPE_DICT[k] for k in list(TYPE_DICT)[:6]}
        TYPE_DICT_2 = {k: TYPE_DICT[k] for k in list(TYPE_DICT)[6:]}

        subframe = tk.Frame(frame, name='subframe1')
        subframe.pack(fill='x')
        txt_label = tk.Label(subframe, text="type:")
        txt_label.pack(side='left')
        for type_val, type_str in TYPE_DICT_1.items():
        ...
        for type_val, type_str in TYPE_DICT_2.items():
            radio = tk.Radiobutton(subframe, width=width_val, text=type_str, variable=self.type, value=type_val,
                                   command=self.type_select_by_radio)
            radio.pack(side="left", anchor="w")
    ...
    def fill_frame__BOOLS(self, frame):
        ...
        txt_label.pack(side='left')
        checkbox = tk.Checkbutton(frame, text="", variable=self.highlighted, command=self.highlighted_clicked, name='highlighted')
        checkbox.pack(side='left')
    ...
    def fill_frame__info_from_outside(self, frame):
        ...
        button = tk.Button(subframe, width=35, text='Load Element from Virtual MiniWob', name='load_element_from_dom_button',
        ...
        button = tk.Button(subframe, width=30, text='Load Element from Visual Observer', name='load_element_from_scree2text_button',
                           command=self.load_element_from_screen2text)
        button.pack(side='left')

    def load_element_from_dom(self):
        annotator_support.load_element_from_dom(self)

    def load_element_from_screen2text(self):
        annotator_support.load_element_from_screen2text(self)

    def element_descriptions(self):
        annotator_support.element_descriptions(self)

    def masking_by_type(self):
        annotator_support.masking_by_type(self)
