...
def ask_main_controller(self, req):
    ...
    elif req == "exclude_prompt_type":
        return self.exclude_prompt_type    
    ...
    elif req == "open_Prompter":
        return self.open_Prompter
...
def start_run(self):
    ...
    for project in PROJECT_LIST:
        ...
        self.exclude_prompt_type = project['exclude_prompt_type']
            ...
            for seed in seed_list:
                try:
                    ...
                    if "_PASS" in self.LLMRecorder.job_status:
                        count_for_pass+=1
                    txt1.config(text=f"{done_cnt} / {run_cnt} ({round(100*done_cnt/run_cnt)} %)  PASS {count_for_pass} ({round(100*count_for_pass/run_cnt)} %)")
                    time_str = str(timedelta(seconds=round(time.time() - start_time)))
                    txt2.config(text=f"Elapsed: {time_str}")
                    self.win.update()

                    result_dict[f"{task}__seed_{seed}"] = f"JOB STATUS : {self.LLMRecorder.job_status} Elapsed: {time_str} "
                except Exception:
        ...
        run_result_file = os.path.join(self.LLMRecorder.project_folder, f'run_result_{time_tag}.txt')
        ...
        if total_num > 0:
            with open(run_result_file, 'w', encoding='utf-16') as file:
                ...
                file.write(f"VISUALOBSERVER_COMMENTATOR : {project['VISUALOBSERVER_COMMENTATOR']}\n")
                file.write(line_divider)
                
                for key, value in result_dict.items():
                    file.write(f"{key}: {value}\n")

                file.write(f"PASS : {count_for_pass}, FAIL : {total_num-count_for_pass}, Success rate : {(count_for_pass/total_num)}\n")
                file.write(line_divider)
...
def update_env_list(self):
    ...
    if os.path.isfile(self.external_env_list_path):
        ...
        self.env_list = [line for line in tmp_env_list if len(line) > 0 and not (line.startswith('#') or line.startswith('/'))]
