...
# click, control+click, point, breakdown
search_pattern=r'Argument:\s*\{\s*element_id\s*:\s*(?P<yy>.+?)\s*\}'
...
def new_project(self):
    ...
    create_button = tk.Button(frame, width=15, height=2, text="Create", command=lambda: (new_project_name(self, new_name=popup.name_entry.get()), popup.destroy()))
...
def new_project_name(self, new_name:str):
    time_tag = datetime.now(pytz.timezone("Asia/Seoul")).strftime("%Y%m%d_%H%M%S")
    if new_name=="":
...
def new_job(self):
    ...
    pyautogui.moveTo(1+win_x, 1+win_y)
    pyautogui.click(1+win_x, 1+win_y)
    ...
    source_file = os.path.join("./functions", default_functions_json)
    destination_file = os.path.join(self.functions_folder, default_functions_json)
...
def _update_action_list_with_imported_element_list(self):
    ...
    if 'error' in action or action['name'] == 'breakdown':
    ...
    if 'error' in action:
        for k in ['num_screen_elements', 'breakdown', 'sub_element_list', 'num_breakdown_sub_elements']:
            if k in previous_action:
                action[k] = previous_action[k]
    else:
        element_list = get_sorted_elements(self.job_folder, self.action_list, action_idx, include_breakdown=False)
        action['num_screen_elements'] = len(element_list)

        # Continued Breakdown case
        action_name = action['name']
        if action_name in ['drag_mouse_hold_down', 'drag_mouse_move', 'drag_mouse_release']:
            previous_action = self.action_list[action_idx-1]
            if 'breakdown' in previous_action.keys():
                old_element = json.loads(previous_action['breakdown'])
                element_id = elements_util.find_matching_element(element_list, old_element)
                if element_id >= 0:
                    action['breakdown'] = json.dumps(element_list[element_id])
        if action_name in ['click_element', 'point_element']:
            # continue breakdown only when the clicked element is breakdown type
            element = json.loads(action['element'])
            if element['type'] == 100:
                previous_action = self.action_list[action_idx-1]
                old_element = json.loads(previous_action['breakdown'])
                element_id = elements_util.find_matching_element(element_list, old_element)
        ...
        # Process Breakdown
        if 'breakdown' in action.keys():
            element = json.loads(action['breakdown'])

            png_fullpath = os.path.join(self.job_folder, action['final_image'])
            sub_element_list = Breakdown(element, png_fullpath)

            action['sub_element_list'] = sub_element_list
            action['num_breakdown_sub_elements'] = len(sub_element_list)
...
def _execute(self, action_str):
    ...
    action = match[0]
    if action == "type_text":
        match = re.search(r'Argument:\s*\{\s*string_to_type\s*:\s*(?P<yy>.+?)\s*\}', action_str)
        ...
        _execute__type_text(self, string_to_type)

    elif action == "press_control_A":
        _execute__press_control_A(self)

    elif action == "press_control_C":
        _execute__press_control_C(self)

    elif action == "press_control_V":
        _execute__press_control_V(self)

    elif action == "click_element":
        match = re.search(search_pattern, action_str)
        element_id = int(match.group('yy'))
        _execute__click_element(self, element_id)

    elif action == "control_click_element":
        match = re.search(search_pattern, action_str)
        element_id = int(match.group('yy'))
        _execute__control_click_element(self, element_id)

    elif action == "point_element":
        match = re.search(search_pattern, action_str)
        element_id = int(match.group('yy'))
        _execute__point_element(self, element_id)

    elif action == "drag_mouse_hold_down":
        match = re.search(r'Argument:\s*\{\s*x\s*:\s*(?P<xx>.+?),\s*y\s*:\s*(?P<yy>.+?)\s*\}', action_str)
        x_loc = int(match.group('xx'))
        y_loc = int(match.group('yy'))
        _execute__drag_mouse_hold_down(self, x_loc, y_loc)

    elif action == "drag_mouse_move":
        match = re.search(r'Argument:\s*\{\s*x\s*:\s*(?P<xx>.+?),\s*y\s*:\s*(?P<yy>.+?)\s*\}', action_str)
        x_loc = int(match.group('xx'))
        y_loc = int(match.group('yy'))
        _execute__drag_mouse_move(self, x_loc, y_loc)

    elif action == "drag_mouse_release":
        _execute__drag_mouse_release(self)

    elif action == "breakdown":
        match = re.search(search_pattern, action_str)
        element_id = int(match.group('yy'))
        _execute__breakdown(self, element_id)
...
def _execute__type_text(self, string_to_type):
    ...
    if not self.FLAG__disable_record:
        ...
        action['script'] = f"action_{action_idx+2}: {{name: type_text, arg: '{string_to_type}'}}"
        action['final_image'] = f"action_{action_idx+2}__type_text.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__press_control_A(self):
    ...
    if not self.FLAG__disable_record:
        # Action Record
        action_idx = round(self.selected/2 - 0.25)
        action = {'name': 'press_control_A'}
        action['script'] = f"action_{action_idx+2}: {{name: press_control_A, arg: {{}}}}"
        action['final_image'] = f"action_{action_idx+2}__cntl_a.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__press_control_C(self):
    ...
    if not self.FLAG__disable_record:
        # Action Record
        action_idx = round(self.selected/2 - 0.25)
        action = {'name': 'press_control_C'}
        action['script'] = f"action_{action_idx+2}: {{name: press_control_C, arg: {{}}}}"
        action['final_image'] = f"action_{action_idx+2}__cntl_c.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__press_control_V(self):
    ...
    if not self.FLAG__disable_record:
        # Action Record
        action_idx = round(self.selected/2 - 0.25)
        action = {'name': 'press_control_V'}
        action['script'] = f"action_{action_idx+2}: {{name: press_control_V, arg: {{}}}}"
        action['final_image'] = f"action_{action_idx+2}__cntl_v.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__click_element(self, element_id):

    # Execute Action
    action_idx = round(self.selected/2 - 0.25)
    element_list = get_sorted_elements(self.job_folder, self.action_list, action_idx, visible_only=False)
    if _handle_error(self, element_id, element_list, 'click_element', None):
        return
    ...
    if not self.FLAG__disable_record:
        ...
        action['script'] = f"action_{action_idx+2}: {{name: click_element, arg: {elements_util.to_string(element)}}}"
        action['final_image'] = f"action_{action_idx+2}__click.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__control_click_element(self, element_id):

    # Execute Action
    action_idx = round(self.selected/2 - 0.25)
    element_list = get_sorted_elements(self.job_folder, self.action_list, action_idx, visible_only=False)
    if _handle_error(self, element_id, element_list, 'control_click_element', None):
        return
    ...
    if not self.FLAG__disable_record:
        # Action Record
        action = {'name': 'control_click_element'}
        action['element'] = json.dumps(element)
        action['script'] = f"action_{action_idx+2}: {{name: control_click_element, arg: {elements_util.to_string(element)}}}"
        action['final_image'] = f"action_{action_idx+2}__control_click.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__point_element(self, element_id):

    # Execute Action
    action_idx = round(self.selected/2 - 0.25)
    element_list = get_sorted_elements(self.job_folder, self.action_list, action_idx, visible_only=False)
    if _handle_error(self, element_id, element_list, 'point_element', None):
        return
    ...
    if not self.FLAG__disable_record:
        # Action Record
        action = {'name': 'point_element'}
        action['element'] = json.dumps(element)
        action['script'] = f"action_{action_idx+2}: {{name: point_element, arg: {elements_util.to_string(element)}}}"
        action['final_image'] = f"action_{action_idx+2}__point.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__drag_mouse_hold_down(self, x, y):
    ...
    if not self.FLAG__disable_record:
        ...
        script = f"action_{action_idx+2}: {{name: drag_mouse_hold_down, arg: {{x={x}, y={y}}}"
        if element:
            script += f", element: {elements_util.to_string(element)}"
        script += "}"

        # Action Record
        action = {'name': 'drag_mouse_hold_down'}
        action['element'] = json.dumps(element)
        action['script'] = script
        action['final_image'] = f"action_{action_idx+2}__drag_mouse_hold_down.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__drag_mouse_move(self, x, y):
    ...
    if not self.FLAG__disable_record:
        ...
        # Action Record
        action_idx = round(self.selected/2 - 0.25)
        action = {'name': 'drag_mouse_move'}
        action['xy'] = (x, y)
        action['script'] = f"action_{action_idx+2}: {{name: drag_mouse_move, arg: {{x={x}, y={y}}}, displacement:{{x_direction={x_shift}, y_direction={y_shift}}}}}"
        action['final_image'] = f"action_{action_idx+2}__drag_mouse_move.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__drag_mouse_release(self):
    ...
    if not self.FLAG__disable_record:
        ...
        # Action Record
        action_idx = round(self.selected/2 - 0.25)
        action = {'name': 'drag_mouse_release'}
        action['script'] = f"action_{action_idx+2}: {{name: drag_mouse_release, arg: {{}}}}"
        action['final_image'] = f"action_{action_idx+2}__drag_mouse_release.png"
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def _execute__breakdown(self, element_id):

    # Execute Action
    action_idx = round(self.selected/2 - 0.25)
    element_list = get_sorted_elements(self.job_folder, self.action_list, action_idx, visible_only=False)
    if _handle_error(self, element_id, element_list, 'breakdown', None):
        return

    element = element_list[element_id]
    if not self.FLAG__disable_record:
        # Action Record
        action = {'name': 'breakdown'}
        action['script'] = f"action_{action_idx+2}: {{name: breakdown, arg: {elements_util.to_string(element)}}}"
        action['final_image'] = f"action_{action_idx+2}__breakdown.png"
        action['breakdown'] = json.dumps(element)
        self.action_list = self.action_list[:action_idx+1]  # trim, if future actions exist
        self.action_list.append(action)
        _dump_action_to_json(self)
...
def update(self):
    ...
    for idx, action in enumerate(self.action_list):
        ...
        if 'num_screen_elements' in action:
            ...
            if 'breakdown' not in action.keys():
                desc = f'   └─ found elements: {num_el}'
            else:
                num_sub_el = action['num_breakdown_sub_elements']
                desc = f'   └─ found elements: {num_el} + {num_sub_el} (breakdown)'
            listbox.insert('end', desc)
