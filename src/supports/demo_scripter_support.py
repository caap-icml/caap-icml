...
'''
    ...
    elif _breakdown_case(self, sel):
        _apply_breakdown(self, sel)
        return
        ...
        if action_name == "click":
            ...
            if element_id < 0:
                raise CustomException(message_element_not_found)
        ...
        elif action_name == "breakdown":  # This is a funny case where apply_breakdown has already happend, and the user is re-running convert on it.
            x, y = action['pos']
            element_list = get_sorted_elements(self.record_folder, self.action_list, sel - 1)
            element_id = elements_util.find_by_xy(element_list, x, y)
            if element_id < 0:
                raise CustomException("no breakdown element detected?")
            element = element_list[element_id]
            element_desc = elements_util.to_string(element)
            script = f"action_{sel+1}: {{name: breakdown, arg: {element_desc}}}"
            breakdown = element
            png_fullpath = os.path.join(self.record_folder, action['final_image'])
    ...
    # Action_List Update
    #####################################################
    if _continued_breakdown_case(self, sel):
        element_list = get_sorted_elements(self.record_folder, self.action_list, sel - 1, include_breakdown=False)
        old_element = self.action_list[sel-1]['breakdown']
    ...
    for future_action in self.action_list[sel+1:]:
        ...
        future_action['breakdown'] = False
        future_action['sub_element_list'] = []
...
def _breakdown_case(self, sel):

    previous_action = self.action_list[sel-1]
    if previous_action['breakdown']:
        return False

    action = self.action_list[sel]
    action_name = action['name']
    if action_name in ['click', 'point', 'mouse_down']:
        if action_name == "click":
            x, y = action['pos1']
        elif action_name == "point":
            x, y = action['pos']
        ...
        if element_type in ['scrollbar_v']:
            return True

    return False


def _apply_breakdown(self, sel):
    action = self.action_list[sel]
    previous_action = self.action_list[sel-1]
    ...
    #####################################################
    new_action = {}
    new_action['name'] = 'breakdown'
    new_action['pos'] = (x, y)
    new_action['final_image'] = previous_action['final_image']
    new_action['description'] = f'breakdown ({x}, {y})'
    new_action['script'] = script
    new_action['reason'] = ''
    new_action['breakdown'] = element
    png_fullpath = os.path.join(self.record_folder, new_action['final_image'])
    new_action['sub_element_list'] = Breakdown(element, png_fullpath)
    self.action_list.insert(sel, new_action)
...
def _continued_breakdown_case(self, sel):
    action = self.action_list[sel]
    action_name = action['name']

    # Case 1 - Dragging with Breakdown
    if action_name in ['mouse_down', 'drag_move', 'mouse_up']:
        previous_action = self.action_list[sel-1]
        if previous_action['breakdown']:
            return True

    # Case 1 - Click/Point on Breakdown element
    if action_name in ['click', 'point']:
        if action_name == "click":
            x, y = action['pos1']
        elif action_name == "point":
            x, y = action['pos']
