...
def save_element(self, do_auto_fit=False):
    ...
    element = {'coords': [x1+self.canvas_x_offset, y1+self.canvas_y_offset, x2+self.canvas_x_offset, y2+self.canvas_y_offset],
...
def load_element_from_screen2text(self):
    main_screen2text = self.ask_main_controller("VisualObserver")
    self.element_list = main_screen2text.element_list.copy()
    dump_annotation_file(self)
    self.update()
...
def make_clean_element_list(self):
    ...
    for element in self.element_list:
        if "type" not in element.keys():
            raise CustomException(f"data something wrong : {element}")
...
def masking_by_type(self):
    ...
    for subframe_name in ['subframe1', 'subframe2']:
