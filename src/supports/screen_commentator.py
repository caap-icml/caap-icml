...
def _get_highlight_image(img, bbox):
    # Make highlight image
    width = img.width
    height = img.height
    x1, y1, x2, y2 = bbox
    new_img = Image.new('RGBA', (width, height), (255, 255, 255, 0))
    new_img.paste(img, (0, 0))
    mask = Image.new('RGBA', (width, height), (0, 0, 0, 50))
    draw = ImageDraw.Draw(mask)

    draw.rectangle((x1, y1, x2, y2), fill=(255, 255, 255, 0))
    new_img.paste((0, 0, 0, 100), mask=mask)
    return new_img.copy()
...
class ScreenCommentator:
    def __init__(self, saved_chechpoint_root_path):
        ...
        checkpoint_base_path = os.path.join(saved_chechpoint_root_path, 'saved_checkpoint')

        # Set Detector & Commentator checkpoint base
        self.detector_checkpoints_base = os.path.join(checkpoint_base_path, 'detectors')
        self.commentator_checkpoint_base = os.path.join(checkpoint_base_path, 'commentator')
    ...
    def _load_detector_models(self):
        detector_name_list = os.listdir(self.detector_checkpoints_base)
        self.detector_list = []
        for detector_name in detector_name_list:
            checkpoint_path = os.path.join(self.detector_checkpoints_base, detector_name)
            self.detector_list.append(YOLO(checkpoint_path))
    ...
    def _get_wbfed_bbox_from_object_detectors(self, image, width, height, score_thershold=0.0):
        # Each detector predict bboxes
        result_list = []
        for detector in self.detector_list:
            results = detector.predict(image, conf=0.7, iou=0.4, verbose=False)
            result_list.append(results)

        # Split the result into box info, scores, and labels.
        boxes_list = []
        scores_list = []
        labels_list = []
        weights = []
        for result in result_list:
            boxes_list.append(result[0].boxes.xyxyn.tolist())
            scores_list.append(result[0].boxes.conf.tolist())
            labels_list.append(result[0].boxes.cls.tolist())
            weights.append(1.0)
        iou_thr = 0.5
        skip_box_thr = 0.0001
        
        boxes, scores, _ = weighted_boxes_fusion(boxes_list, scores_list, labels_list, weights=weights, iou_thr=iou_thr, skip_box_thr=skip_box_thr)
        result_bbox_list = []
        for idx, bbox in enumerate(boxes):
            if scores[idx] < score_thershold:
                continue
            origal_bbox = [bbox[0] * width, bbox[1] * height, bbox[2] * width, bbox[3] * height]
            result_bbox_list.append(origal_bbox)
    ...
    def run_models(self, target_image, multiplier):
        ...
        bbox_list = self._get_wbfed_bbox_from_object_detectors(target_image, width, height)
