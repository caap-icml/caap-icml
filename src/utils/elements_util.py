...
KEYS_USED = {
    ...
    'radio': BASIC_KEYS + ['checked'],
    'checkbox': BASIC_KEYS + ['checked'],
...
def to_string(element, add_visible=False):
    ...
    element_type = TYPE_DICT[type_num]
    pair_list.append(f"type: {element_type}")
    ...
    for key in KEYS_USED[element_type]:
        if key == 'text':
            if 'text' in element:
                pair_list.append(f'text: "{element["text"]}"')
            else:
                pair_list.append('text: ""')

        if key == 'checked':
            if 'checked' in element and element["checked"] == "True":
                pair_list.append('checked: True')

        if key == 'focused':
            if 'focused' in element and element["focused"] == "True":
                pair_list.append('focused: True')

        if key == 'highlighted':
            if 'highlighted' in element and element["highlighted"] == "True":
                pair_list.append('highlighted: True')
...
def find_by_xy(element_list, x, y):
    ...
    if len(found_id_list) > 1:
        if all(x == found_type_list[0] for x in found_type_list):
            ...
            found_id = found_id_list[min_index]
...
def get_element_list_from_annotation_file(job_folder, action_list, action_idx, include_breakdown=True):
    ...
    if include_breakdown:
        ...
        if 'breakdown' in action:
            sub_element_list = action['sub_element_list']
            element_list += sub_element_list
...
def dump_screen_history(job_folder, action_list, action_idx):
    screen_history = get_screen_history(job_folder, action_list, action_idx, include_breakdown=True)
...
def get_screen_history(job_folder, action_list, action_idx, include_breakdown=True):
    if action_idx < 0:
        return []

    curr = get_element_list_from_annotation_file(job_folder, action_list, action_idx, include_breakdown=include_breakdown)
...
def get_sorted_elements(job_folder, action_list, action_idx, include_breakdown=True, visible_only=True):
    screen_history = get_screen_history(job_folder, action_list, action_idx, include_breakdown=include_breakdown)
